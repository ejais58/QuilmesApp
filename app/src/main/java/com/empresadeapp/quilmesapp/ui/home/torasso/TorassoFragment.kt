package com.empresadeapp.quilmesapp.ui.home.torasso
...
import com.empresadeapp.quilmesapp.databinding.FragmentTorassoBinding
...
class TorassoFragment : Fragment(R.layout.fragment_torasso) {

    private lateinit var binding: FragmentTorassoBinding
    private val viewModel by viewModels<HomeViewModel>{ HomeViewModelFactory(HomeRepoImpl(HomeDataSource())) }

    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        super.onViewCreated(view, savedInstanceState)
        binding = FragmentTorassoBinding.bind(view)
        recyclerTorasso()

    }

    private fun recyclerTorasso(){
        viewModel.fetchProductoTorasso().observe(viewLifecycleOwner, Observer { result ->
            when (result){
                is Result.Loading -> {
                ...
                is Result.Success -> {
                    ...
                    binding.rvTorasso.adapter = GlobalAdapter(result.data)
                }
                is Result.Failure -> {
                    binding.progressBar.visibility = View.GONE
